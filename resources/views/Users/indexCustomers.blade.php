...
<div class="d-flex" id="wrapper">
    ...
    <div class="container text-center">
        ...
        <div class="row text-left">
            <a href="{{route('customer.create')}}" style="color: #fff;"><button class="btn text-left btn-danger">Nuevo Cliente</button></a>
        </div>
        ...
        <table class="table" style="margin-top: 50px;" >
            ...
            @foreach($users as $user)
                @if($user->Enterprise_id == Auth::user()->Enterprise_id && $user->roles()->first()->rol == "Customer")
                    <tr>
                        <td>{{$user->Name}}</td>
                        <td>{{$user->LastName}}</td>
                        <td>{{$user->email}}</td>
